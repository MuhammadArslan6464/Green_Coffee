    ...
    if (isset($_GET['get_id'])) {
        ...
        if ($get_product->rowCount() > 0) {
            while ($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)) {
                ...
                $result = $insert_order->execute([unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $fetch_p['id'], $fetch_p['price'], 1]);
                if ($result) {
        ...
        while ($f_cart = $varify_cart->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($select_products->rowCount() > 0) {
                ...
                $insert_order = $conn->prepare("INSERT INTO `orders` (id, user_id, name, number, email, address, address_type, method, product_id, price, qty) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $result = $insert_order->execute([unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $f_cart['product_id'], $fetch_products['price'], $f_cart['qty']]);
                // $insert_order = $conn->prepare("INSERT INTO `orders` (id, user_id, name, number, email, address, address_type, method, product_id, price, qty) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                // $result = $insert_order->execute([unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $f_cart['product_id'], $fetch_products['price'], $f_cart['qty']]);
                if ($result) {
...
<div class="banner">
    ...
    <section class="checkout">
        <div class="title">
            ...
            <h1>Checkout summary</h1>
            <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Nisi, delectus ea? Accusamus eveniet maxime facilis!</p>
        </div>
        <div class="row">
            <form method="post">
                    ...
                    if(isset($_GET['get_id'])) {
                        ...
                        while($fetch_get = $select_get->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            <div>
                                ...
                                <h3 class="price"><?=$fetch_get['price']; ?>/-</h3>
                            </div>
                            </div>
                            <?php
                        }
                        ...
                        if($select_cart->rowCount() > 0) {
                            while($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                <h3 class="price"><?=$fetch_products['price']; ?> X <?=$fetch_cart['qty']; ?></h3>
                                </div>
                                </div>
                                <?php
                            }
